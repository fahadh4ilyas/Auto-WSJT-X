...
def filter_cq(data: dict, states: States) -> bool:
    ...
    if not data['isNewCallsign']:
        logging.warning(f'[HOST: {states.namespace}] Already QSO with this callsign')
        return False

    if data['SNR'] < states.min_db:
        logging.warning(f'[HOST: {states.namespace}] The message\'s signal is below minimum threshold')
        return False

    if DXCC_EXCEPTION and 'dxcc' in data and data['dxcc'] in DXCC_EXCEPTION:
        logging.warning(f'[HOST: {states.namespace}] The callsign is inside DXCC exception')
        return False

    if data.get('extra', None):
        if (data['extra'] == 'DX' and data.get('country', '') == 'Indonesia') or data['extra'] != 'OC':
            logging.warning(f'[HOST: {states.namespace}] The callsign specifically didn\'t want to call us')
            return False
...
def process_wsjt(_data: bytes, ip_from: tuple, states: States):
    ...
    if isinstance(packet, wsjtx.WSHeartbeat):
        ...
        logging.debug(f'[HOST: {ip_from[0]}:{ip_from[1]}] '
            f'[MY CALLSIGN: {packet.DeCall}] [MY GRID: {packet.DeGrid}] '
        ...
        if data['type'] == 'CQ':
            ...
            if data['to'] == LOCAL_STATES['my_callsign']:
                ...
                if latest_data:
                    if latest_data.get('to', None) == LOCAL_STATES['my_callsign'] and latest_data.get('R73', None) != '73':
                        logging.warning(f'[HOST: {ip_from[0]}:{ip_from[1]}] Sending Grid to other callsign even though still talking with me!')
                        if latest_data['tried'] and latest_data['nextTx'] == 'R73':
                            return
                        if not (latest_data['tried'] and latest_data['isReemerging']):
                            if latest_data['tried']:
            ...
            if data['to'] == LOCAL_STATES['my_callsign']:
                ...
                if latest_data: 
                    if latest_data.get('to', None) == LOCAL_STATES['my_callsign'] and latest_data.get('R73', None) != '73':
                        logging.warning(f'[HOST: {ip_from[0]}:{ip_from[1]}] Sending signal to other callsign even though still talking with me!')
                        if latest_data['tried'] and latest_data['nextTx'] == 'R73':
                            return
                        if not (latest_data['tried'] and latest_data['isReemerging']):
                            if latest_data['tried']:
            ...
            if data['to'] == LOCAL_STATES['my_callsign']:
                ...
                if latest_data:
                    if latest_data.get('to', None) == LOCAL_STATES['my_callsign'] and latest_data.get('R73', None) != '73':
                        logging.warning(f'[HOST: {ip_from[0]}:{ip_from[1]}] Replying signal to other callsign even though still talking with me!')
                        if latest_data['tried'] and latest_data['nextTx'] == 'R73':
                            return
                        if not (latest_data['tried'] and latest_data['isReemerging']):
                            if latest_data['tried']:
